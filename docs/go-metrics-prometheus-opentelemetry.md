...
### 3.5 Metrics 是否必须暴露一个端口

不一定，关键不在于“是否单独开一个新端口”，而在于“观测系统能否拿到指标数据”。

如果你使用的是 Prometheus 最常见的 pull 模式，那么通常需要提供一个 Prometheus 可以访问的 metrics endpoint，例如 `/metrics`。但这不等于必须额外开一个专门的新端口，常见做法有两种：

- 和业务接口共用同一个端口，只是在同一个服务里增加一个 `/metrics` 路径。
- 单独开一个管理端口，例如业务接口走 `:8080`，metrics 走 `:9090`。

所以在 Prometheus pull 模式下，更准确的说法是：

- 通常需要一个可访问的 metrics endpoint。
- 但不一定需要一个独立的 metrics 端口。

如果你使用的是 OpenTelemetry 的 push 风格链路，例如应用将 metrics 通过 OTLP 主动发给 OpenTelemetry Collector，那么应用本身可以不暴露 `/metrics`。这类常见链路通常是：

应用埋点 -> OTel SDK 聚合 -> OTLP Exporter -> OTel Collector -> 观测后端

在这种模式下，应用不一定需要暴露任何 metrics HTTP 接口。

工程上常见的选择通常是：

- 内网中的简单服务，直接在业务端口挂 `/metrics`，实现和排查都更直接。
- 对隔离和安全要求更高的服务，单独暴露管理端口。
- 已经建设统一可观测性平台的团队，应用通过 OTLP 上报到 Collector，而不是直接对外暴露 `/metrics`。

因此，“是否必须暴露一个端口才能观察到 metrics”并没有统一答案，它取决于你的采集模型：

- Prometheus pull：通常需要可访问的 endpoint，但不一定是独立端口。
- OpenTelemetry push：应用本身可以不暴露 metrics 端口。
